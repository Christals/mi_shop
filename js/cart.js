function fn() {
    ...
    var $Gou = document.querySelectorAll('.col-check ');
    console.log($Gou);
	var priceAll = 0;
	for(var k = 0; k < $Gou.length; k++) {
		if($Gou[k].className == 'Gou active') {
			var tt = $($Gou[k]);
			var jiage = tt.parent().parent().find('.col-price').text();
			var shuLiang = tt.parent().parent().find('.col-num').text();
...
$(function() {
	if(localStorage.getItem('xiaomi')) {
		...
		for(var i = 0; i < arr.length; i++) {
			...
			obj = arr[i];
			console.log(obj)
            var shangPing = `<div class="item-row">
                            <div class="col col-check">
                                <i class="iconfont icon-check">√</i>
                            </div>
                            <div class="col col-img">
                                <img src="${obj.phoneurl}" alt="">
                            </div>
                            ...
                            <div class="col col-num">
                                <div class="change-num">
                                    ...
                                    <input type="text" class="goods-num" value="${obj.tok}">
